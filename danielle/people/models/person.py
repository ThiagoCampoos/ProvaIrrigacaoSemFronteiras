...
class Person(BaseModel):
    """Entidade de pessoa com dados cadastrais e validacoes brasileiras."""
    ...
    email = models.EmailField(
        max_length=100, blank=True, null=True, unique=True, verbose_name="E-mail"
    )
    GENDER_CHOICES = [
        ("M", "Masculino"),
        ("F", "Feminino"),
    ...
    rg_ssp = models.CharField(
        max_length=2, choices=STATE_CHOICES, blank=True, null=True, verbose_name="SSP"
    )
    ...
    neighbourhood = models.CharField(
        max_length=60, blank=True, null=True, verbose_name="Bairro"
    )
    ...
    def formatted_born_date(self):
        """Retorna data de nascimento formatada como DD/MM/YYYY."""
        if self.born_date:
    ...
    def formatted_cpf(self):
        ...
        if self.cpf:
            return "{}.{}.{}-{}".format(
                self.cpf[:3], self.cpf[3:6], self.cpf[6:9], self.cpf[9:11]
            )

    @property
    def formatted_postal_code(self):
        """Retorna CEP formatado como XXXXX-XXX."""
        if self.postal_code:
